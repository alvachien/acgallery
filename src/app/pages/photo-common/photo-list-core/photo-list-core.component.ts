...
export class PhotoListCoreComponent {
  ...
  onPhotoViewEXIF(pht: Photo): void {
    this.odataSvc.getPhotoEXIF(pht.photoId).subscribe({
      ...
      error: (err) => {
        writeConsole(
          `ACGallery [Error]: Entering PhotoListCoreComponent onPhotoViewEXIF, getPhotoEXIF ${err.toString()}`,
  ...
  onDeletePhoto(pht: Photo): void {
    ...
    this.odataSvc.deletePhoto(pht.photoId).subscribe({
      ...
      error: (err) => {
        writeConsole(
          `ACGallery [Error]: Entering PhotoListCoreComponent onDeletePhoto, deletePhoto ${err.toString()}`,
          ConsoleLogTypeEnum.error
        );
      },
    });
